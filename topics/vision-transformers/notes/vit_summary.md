...
- **MAE (2022)**：隨機 mask 75% patches，只讓 encoder 看到可見 patches，decoder 重建 pixel。**改變了 ViT 的 pre-training 範式**（discriminative → generative）→ 見 Step 9
- **I-JEPA (2023)**：改進 MAE 的「predict pixels」→「predict latent representation」，避免學習低層次 pixel 細節，JEPA 系列的起點 → 見 Step 8
...
> **→ 前置知識**：Attention 機制詳解，見 [attention_transformer.md](./attention_transformer.md)
> **→ 後續閱讀**：MAE 如何用 ViT 做 self-supervised pre-training（Step 9）
> **→ 後續閱讀**：I-JEPA 如何在 latent space 做 masked prediction（Step 8）
