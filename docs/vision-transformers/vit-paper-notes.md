...
```mermaid
graph LR
    A["Input<br/>(B, C, H, W)"] --> B["Patch Split<br/>N = (H/P)×(W/P)"]
    B --> C["Linear Projection<br/>(B, N, D)"]
    C --> D["Prepend [CLS]<br/>(B, N+1, D)"]
    D --> E["+ Position Emb<br/>(learnable, 1D)"]
    E --> F["Transformer<br/>Encoder × L"]
    F --> G["CLS output z₀ᴸ"]
    G --> H["MLP Head<br/>→ logits"]
```
...
!!! warning "面試常考"
    Position embedding 的 2D 插值是 Section 3.2 的核心工程細節。
...
!!! note "計算量公式"
    Self-attention 計算量為 $O(N^2 \cdot D)$，其中 $N = (H/P)^2$。Patch 越小，$N$ 越大，計算量平方增長。
...
!!! tip "14M 是分水嶺"
    資料不足時，ViT 缺少 CNN 的 locality/spatial bias，表現更差。超過 14M 後，scale 的優勢壓過 inductive bias 的劣勢。
...
```mermaid
graph TD
    V["ViT (2020)<br/>Pure Transformer for vision"] --> D["DeiT (2021)<br/>Knowledge distillation<br/>No JFT needed"]
    V --> M["MAE (2022)<br/>Mask 75% patches<br/>Predict pixels"]
    M --> J["I-JEPA (2023)<br/>Predict latent repr.<br/>Not pixels"]

    style V fill:#4051b5,color:#fff
    style D fill:#26a69a,color:#fff
    style M fill:#26a69a,color:#fff
    style J fill:#26a69a,color:#fff
```

## Interview Q&A

??? question "ViT 為什麼需要大規模資料預訓練？"
    CNN 的 locality（只看鄰近像素）和 translation equivariance（位移不變性）是 inductive bias，等於免費給模型注入了圖像先驗知識。ViT 沒有這些 bias，必須從資料中從頭學習。小資料集 (~1M) 不夠讓 ViT 學習這些先驗，14M+ 才能讓 scale 的優勢勝出。

??? question "CLS token 的作用，為何不用 GAP？"
    CLS token 在預訓練中已學習 attend over 全部 patches，成為全局表示。Fine-tune 時只需替換 MLP head，訓練一致。GAP 需要對所有 patch embedding 平均，有時效果接近，但實作上 CLS 更自然（繼承 BERT 設計）。

??? question "Fine-tune 時解析度更高，position embedding 怎麼處理？"
    預訓練 position embedding 形狀不符（$N_{train} \neq N_{finetune}$）。標準做法：把 1D embedding reshape 回 2D ($H/P \times W/P$)，用 bilinear interpolation 插值到新解析度的 grid，再 flatten 回 1D。這樣保留了相對位置資訊。

??? question "ViT 的 patch size 如何影響效能？"
    patch 越小（如 $P=14$ vs $P=16$），sequence 越長（$N$ 更大），計算量 $O(N^2)$ 更大，但能捕捉更細緻的局部資訊。ViT-H/14 比 ViT-L/16 在高解析度任務上更強，但訓練更貴。

??? question "為什麼 1D position encoding 效果等同於 2D？"
    Transformer 的 self-attention 讓模型能透過觀察所有 patches 之間的 attention pattern 隱式學習 2D 空間關係，不依賴顯式的 2D structure。論文消融實驗顯示三種 PE 差距微小。

!!! abstract "前置知識"
    Attention 機制詳解，見 [Attention Is All You Need](attention-paper-notes.md)
